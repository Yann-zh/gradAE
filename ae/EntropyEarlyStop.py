...
class EntropyEarlyStop:
    ...
    def __init__(self, k=100, R_down=0.1):
        self.k = k
        self.R_down = R_down
        self.entropys = []
        self.patience = 0
...
class ModelEntropyEarlyStop:
    ...
    def step(self, entropy_value, model):
        if self.isStop:
            return True
        self.entropys.append(entropy_value)
        if self.minEn is None:
...
def getEntropyStopOffline(En, k=100, R_down=0.01):  # an offline version of EntropyEarlyStop
    ES = EntropyEarlyStop(k, R_down)
    for i in range(len(En)):
        isStop = ES.step(En[i])
        if isStop:
            break
    return ES.getBestEpoch()
